import { Component, OnInit, ViewChild, Input, Output,  EventEmitter, SimpleChanges, SimpleChange
} from '@angular/core';
  ...
  geocode(postcode) {
    ...
    geocoder.geocode( { 'address':  postcode}, (results, status) =>  {
      if (status == google.maps.GeocoderStatus.OK) {
        this.latLng.emit({'lat' : results[0].geometry.location.lat(),'lng' : results[0].geometry.location.lng()});
        let location = new google.maps.LatLng(results[0].geometry.location.lat(), results[0].geometry.location.lng());
